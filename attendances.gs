...
function setMonthlyLastRow() {
  var attendanceSheet = SpreadsheetApp.openById("11GyynG3eJcscLpMpsjvA4yyPKCMe6bBKR3ctwmvRqEo");
  var rowCounts = attendanceSheet.getLastRow();
  attendanceSheet.getRange("I1").setValue(rowCounts-1);
...
function mailMonthlyLeaveDetails(){
  var attendanceSheet = SpreadsheetApp.openById("11GyynG3eJcscLpMpsjvA4yyPKCMe6bBKR3ctwmvRqEo");
  var rowCount = attendanceSheet.getLastRow();
  var startRow = attendanceSheet.getRange("I1").getValue();
  Logger.log(" total row : "+rowCount);
  Logger.log(" start row : "+startRow);
  var todaysDate = new Date();
  var htmlContent = '<p> <b> <center> <H2> K.S.C.C.F Ltd </H2> </center> </b> </p> <p><center> <b> KOZHIKODE REGION  </b></center> </p> <p><center> <b> EMPLOYEES LEAVE ON '+todaysDate+' </b> </center> </p>'
...
function changeSheetDaily(){
  ...
  var startRow = 3;
  var rowCount = attendanceSheet.getLastRow();
  var rowLimit = rowCount -  4;
  attendanceSheet.getSheetByName("ATTENDANCE").hideRows(startRow,rowLimit);
  var lastColumn = attendanceSheet.getLastColumn();
  var columnToHide = attendanceSheet.getRange("G:"+alphaList[lastColumn]);
  attendanceSheet.hideColumn(columnToHide);
  attendanceSheet.unhideColumn(attendanceSheet.getRange("A:F"));
  attendanceSheet.unhideRow(attendanceSheet.getRange("1:2"));
  attendanceSheet.unhideRow(attendanceSheet.getRange((rowCount-1)+":"+rowCount));
  var todayDate = new Date();
  var thisYear = todayDate.getYear();
  var monthList = ["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"];
  ...
  attendanceSheet.getRange("B1").setValue("Date : "+thisYear+" "+monthList[actualMonth]+" "+day+" "+weekList[weekDay]);
  attendanceSheet.getRange("H1").setValue(rowCount-2);
...
function sendEmailwithPdf() {
      ...
      var startRow = sheetToPdf.getRange("H1").getValue();
      var attndCount = startRow + 1;
      var attendanceSheet = sheetToPdf.getSheetByName("ATTENDANCE");
      attendanceSheet.getRange("L2").setValue('=COUNTIF(D'+attndCount+':D,"PRESENT")');
      attendanceSheet.getRange("M2").setValue('=COUNTIF(D'+attndCount+':D,"FULL DAY(L)")'); //ON DUTY
      attendanceSheet.getRange("N2").setValue('=SUM(COUNTIF(D'+attndCount+':D,"HALF DAY - MORNING(L)"),COUNTIF(D5353:D,"HALF DAY - AFTER NOON(L)"))');
      attendanceSheet.getRange("O2").setValue('=COUNTIF(D'+attndCount+':D,"ON DUTY")');
     // attendanceSheet.getRange("P2").setValue(new Date());
      var totPresent = 0;
      var totAbsent = 0;
      var totHalfDays = 0;
      var totOnDuties = 0;
      totPresent = attendanceSheet.getRange("L2").getValue();
      totAbsent = attendanceSheet.getRange("M2").getValue();
      totHalfDays = attendanceSheet.getRange("N2").getValue();
      totOnDuties = attendanceSheet.getRange("O2").getValue();
...
function backupMonthly(){
  ...
  var controllerSheet = SpreadsheetApp.openById("1O9LE0bNoQBnWW33rCzZG6l_EcReY_SQMvZTdRuNQPUg");
  var controlSheet = controllerSheet.getSheetByName("controller");
  var startRow = sheetToPdf.getRange("I1").getValue();
  var rowCount = sheetToPdf.getLastRow();
  ...
  sheetToPdf.getRange("B1").setValue("Month : "+year+" "+months[previousMonth]);
    if(actualMonth==0){
      previousMonth = 11;
    }
